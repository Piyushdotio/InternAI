...
export const InterviewCompletion: React.FC<InterviewCompletionProps> = ({
    ...
    const competencies = [
        { label: 'Technical Accuracy', value: technicalAccuracy },
        { label: 'Communication Clarity', value: communicationClarity },
        { label: 'Problem-Solving Approach', value: problemSolving },
    ];
    ...
    return (
        <div className="max-w-md mx-auto my-6 space-y-4 animate-fade-in pb-12">
            ...
            <div className="bg-white rounded-3xl p-8 shadow-sm border border-gray-200/80 text-center space-y-6 relative overflow-hidden">
                ...
                <div className="relative w-40 h-40 mx-auto flex items-center justify-center">
                    <svg className="w-full h-full transform -rotate-90" viewBox="0 0 120 120">
                        <circle cx="60" cy="60" r={radius} className="text-gray-100" strokeWidth="10" stroke="currentColor" fill="transparent" />
                        <circle
                            cx="60"
                            cy="60"
                            r={radius}
                    ...
                    {competencies.map((comp) => (
                        <div key={comp.label} className="space-y-1.5">
                            <div className="flex justify-between text-xs font-medium">
                                ...
                                <span className="font-bold text-[#2c2e2a]">{comp.value}%</span>
                            </div>
            ...
            <div className="flex items-center gap-3 pt-2">
                <button
                    onClick={onRestart}
                    className="flex-1 bg-white hover:bg-gray-50 border border-gray-300 text-gray-700 font-semibold py-3 px-4 rounded-full text-sm transition-all shadow-xs flex items-center justify-center gap-2 active:scale-95 cursor-pointer"
                >
                    <span>🔄</span> Try Again
                </button>
                <button
                    onClick={onGoToDashboard}
                    className="flex-1 bg-blue-600 hover:bg-blue-700 text-white font-semibold py-3 px-4 rounded-full text-sm transition-all shadow-sm flex items-center justify-center gap-2 active:scale-95 cursor-pointer"
                >
                    Dashboard →
                </button>
            </div>
